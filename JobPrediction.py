...
class JobPrediction:
    ...
    def recommend_new_skills(self, available_skills, target_job, threshold=0):
        ...
        target_results=simulated_results[target_job]
        # print(target_results)
        target_results = target_results.sort_values(ascending=False)
        positive_mask = (target_results > threshold)
        return target_results[positive_mask]

    # ==============================================================
# sample_skills = ['Scala', 'Hadoop', 'Python']
# # # sample_skills =['Java', 'Python', 'R', 'SQL', 'Scala', 'Cassandra', 'MySQL', 'Redis', 'SQLite', 'Flask']
# job_model = JobPrediction(mlflow_uri=MLFLOW_TRACKING_URI,
#                           run_id=MLFLOW_RUN_ID,
#                           clusters_yaml_path=CLUSTERS_YAML_PATH)
# predictions = job_model.recommend_new_skills(sample_skills,'Data scientist or machine learning specialist')
# print((predictions))
